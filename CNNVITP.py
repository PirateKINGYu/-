...
for epoch in range(num_epochs):
    ...
    if val_acc > best_acc:
        best_acc = val_acc
        torch.save(model.state_dict(), 'best_model_from_scratch.pth')
        print('Best model saved.')
    
    print('-' * 30)

# 8. 测试模型
model.load_state_dict(torch.load('best_model_from_scratch.pth'))
...
plt.legend()
plt.grid(True)
plt.savefig('accuracy_curve.png')  # 保存准确率曲线
plt.show()
...
with open('saved_models/Parallel_accuracies.json', 'w') as f:
    json.dump(accuracy_data, f)

print("Validation accuracies saved to 'saved_models/Parallel_accuracies.json'")
