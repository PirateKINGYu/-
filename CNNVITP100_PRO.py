# CNN_ViT_Fusion_CIFAR100_SGD.py
...
for epoch in range(num_epochs):
    ...
    if val_acc > best_acc:
        ...
        torch.save(model.state_dict(), 'saved_models/best_model_fusion_CIFAR100_SGD.pth')
        print('Best model saved.')
    
    print('-' * 30)

# 8. 测试模型
model.load_state_dict(torch.load('saved_models/best_model_fusion_CIFAR100_SGD.pth'))
...
plt.legend()
plt.grid(True)
plt.savefig('saved_models/loss_curve_CIFAR100_SGD.png')  # 保存损失曲线
plt.show()
...
plt.legend()
plt.grid(True)
plt.savefig('saved_models/accuracy_curve_CIFAR100_SGD.png')  # 保存准确率曲线
plt.show()
...
with open('saved_models/Fusion_CIFAR100_SGD_accuracies.json', 'w') as f:
    json.dump(accuracy_data, f)

print("Validation accuracies saved to 'saved_models/Fusion_CIFAR100_SGD_accuracies.json'")
